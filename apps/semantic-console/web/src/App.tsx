...
import type { ColumnRecord, ConsoleSection, CubeSnapshot, Datasource, DatasourceField, DatasourceType, KnowledgeRulesResponse, LocalizedText, McpIntegrationResponse, ProjectDiff, ProjectFile, ProjectSummary, SchemaRecord, SemanticModel, SemanticProjectSnapshot, SemanticRelationship, SqlCandidatesResponse, TableRecord, Theme, ValidationIssue, ValidationResponse, VersionRecord, ViewDefinition, ViewPreviewResult, ViewSnapshot, ViewValidationResponse, ViewWritePayload } from "./types";
import { Badge, Button, EmptyState, Field, InlineNotice, LoadingRows, Modal, Pagination, SectionHeading, Select, TextArea, TextInput, usePagination } from "./components/ui";
...
function App() {
  ...
  async function loadMcpIntegration() {
    setMcpLoading(true);
    setMcpError(null);
    const result = await api.getMcpIntegration().catch((error: unknown) => {
      setMcpError(error instanceof Error ? error.message : "MCP integration could not be loaded.");
      return null;
    });
    if (result) setMcpIntegration(result);
    setMcpLoading(false);
  }
  ...
  const refreshWorkspace = async (): Promise<boolean> => {
    ...
    const protectedResults = [projectResult, datasourcesResult, filesResult, versionsResult];
    const authLocked = !consoleSession && healthOk && protectedResults.every((result) => result.status === "rejected" && result.reason instanceof ApiClientError && (result.reason.status === 401 || result.reason.status === 403));
    setApiOnline(healthOk);
    if (failures.length && !authLocked) {
    ...
    return failures.length === 0 || authLocked;
  };
  ...
  async function resubmitCandidate(candidate: SqlKnowledgeCandidate) {
    await api.resubmitSqlCandidate(candidate.id);
    await loadSqlCandidates();
    markSaved();
  }
  ...
  function navigate(next: ConsoleSection) {
    ...
    if (next === "mcp") void loadMcpIntegration();
  ...
  function markSaved() { const saved = new Date().toISOString(); setLastValidation(null); setDraftSavedAt(saved); setProject((current) => ({ ...current, status: "draft", updatedAt: saved })); setNotice({ tone: "success", title: t("status.draftSaved"), body: t("status.draftSafe") }); }
  ...
  async function saveSqlCandidate(candidate: SqlKnowledgeCandidate, sql: string) {
    await api.updateSqlCandidate(candidate.id, { sql });
    await loadSqlCandidates();
    markSaved();
  }
  async function handleDeleteDatasource() {
    const target = deleteDatasourceTarget;
    if (!target) return;
    setBusyAction(`delete-datasource-${target.id}`);
    const result = await api.deleteDatasource(target.id).catch((error: unknown) => { setNotice({ tone: "error", title: "Data source deletion failed", body: error instanceof Error ? error.message : "The data source could not be deleted." }); return null; });
    setBusyAction(null);
    if (!result) return;
    const remaining = datasources.filter((item) => item.id !== target.id);
    setDatasources(remaining);
    setSelectedDatasourceId(remaining[0]?.id ?? "");
    if (project.activeDatasource?.id === target.id) setProject((current) => ({ ...current, activeDatasource: remaining[0] ?? null }));
    setDeleteDatasourceTarget(null);
    setNotice({ tone: "success", title: "Data source deleted", body: `${target.name} was removed from this project.` });
  }
  ...
  function cancelDatasourceEdit() {
    if (selectedDatasource?.id.startsWith("new-")) {
      const remaining = datasources.filter((item) => item.id !== selectedDatasource.id);
      setDatasources(remaining);
      setSelectedDatasourceId(remaining[0]?.id ?? "");
    }
  ...
  const avatarText = consoleSession?.subjectName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase() || "WS";
  ...
  return <div className="app-shell">
    ...
    <Modal open={Boolean(deleteDatasourceTarget)} title="Delete data source" description="This removes the saved connection profile. Existing semantic files are not deleted." onClose={() => { if (!busyAction?.startsWith("delete-datasource-")) setDeleteDatasourceTarget(null); }} footer={<><Button variant="ghost" onClick={() => setDeleteDatasourceTarget(null)}>Cancel</Button><Button variant="danger" icon={Trash} loading={Boolean(busyAction?.startsWith("delete-datasource-"))} onClick={() => void handleDeleteDatasource()}>Delete data source</Button></>}><InlineNotice tone="warning" title={deleteDatasourceTarget?.name ?? "Data source"}>This action cannot be undone from the Console.</InlineNotice></Modal>
...
function Overview({ project, datasources, versions, files, validation, onNavigate, onHistory }: { project: ProjectSummary; datasources: Datasource[]; versions: VersionRecord[]; files: ProjectFile[]; validation: ValidationResponse | null; onNavigate: (section: ConsoleSection) => void; onHistory: () => void }) {
...
function SchemaPage({ datasources, selectedDatasource, selectedSchema, setSelectedSchema, schemas, tables, search, setSearch, selectedTable, onSelectTable, columns, onImport, busyAction, onRefresh, onDatasourceChange }: { datasources: Datasource[]; selectedDatasource?: Datasource; selectedSchema: string; setSelectedSchema: (value: string) => void; schemas: SchemaRecord[]; tables: TableRecord[]; search: string; setSearch: (value: string) => void; selectedTable: string; onSelectTable: (value: string) => void; columns: ColumnRecord[]; onImport: () => void; busyAction: string | null; onRefresh: () => void; onDatasourceChange: (id: string) => void }) {
  const { pageItems: visibleSchemas, paginationProps: schemaPagination } = usePagination(schemas, selectedDatasource?.id ?? "");
  const { pageItems: visibleTables, paginationProps: tablePagination } = usePagination(tables, `${selectedSchema}|${search}`);
  const { pageItems: visibleColumns, paginationProps: columnPagination } = usePagination(columns, `${selectedSchema}|${selectedTable}`);
  return <div className="page"><SectionHeading eyebrow="Workspace" title="Schema browser" description="Explore source metadata, then import only the tables that belong in your semantic layer." action={<div className="heading-actions"><Button variant="ghost" size="sm" icon={ArrowClockwise} onClick={onRefresh} loading={busyAction === "schema"}>Refresh metadata</Button><Button variant="primary" icon={DownloadSimple} onClick={onImport} disabled={!selectedTable}>Import selected table</Button></div>} /><div className="schema-toolbar"><Field label="Data source" htmlFor="schema-source"><Select id="schema-source" value={selectedDatasource?.id ?? ""} onChange={(event) => onDatasourceChange(event.target.value)}><option value="">Choose a data source</option>{datasources.map((source) => <option key={source.id} value={source.id}>{source.name}</option>)}</Select></Field><div className="schema-toolbar-meta"><span><span className={`source-status ${selectedDatasource ? "source-status-ok" : "source-status-pending"}`} />Metadata is read only</span></div></div><div className="schema-browser"><aside className="schema-sidebar"><div className="schema-sidebar-heading"><h3>Schemas</h3><span>{schemas.length}</span></div><div className="schema-tree">{visibleSchemas.map((schema) => <button className={`schema-tree-item ${schema.name === selectedSchema ? "schema-tree-item-active" : ""}`} key={schema.name} onClick={() => setSelectedSchema(schema.name)}><CaretRight size={14} /><span className="schema-glyph"><Database size={15} /></span><span>{schema.name}</span><small>{schema.tableCount ?? "-"}</small></button>)}{schemas.length === 0 ? <EmptyState icon={Database} title="No schema metadata" body={selectedDatasource ? "Refresh this connection to load its schemas." : "Choose a data source to begin browsing."} /> : null}</div><Pagination {...schemaPagination} /></aside><section className="table-browser"><div className="table-browser-heading"><div><p className="panel-kicker">{selectedSchema || "Select a schema"}</p><h2>Tables and views</h2></div><div className="table-search"><MagnifyingGlass size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter tables" aria-label="Filter tables" /></div></div><div className="table-list">{visibleTables.map((table) => <button className={`table-row ${table.name === selectedTable ? "table-row-active" : ""}`} key={table.name} onClick={() => onSelectTable(table.name)}><span className="table-row-icon">{table.type === "VIEW" ? <Eye size={16} /> : <Table size={16} />}</span><span><strong>{table.name}</strong><small>{table.type === "VIEW" ? "View" : "Table"}</small></span><CaretRight size={15} /></button>)}{tables.length === 0 ? <EmptyState icon={Table} title="No tables found" body={selectedSchema ? "Try a different filter or refresh metadata." : "Choose a schema to load its tables."} /> : null}</div><Pagination {...tablePagination} /></section><section className="column-browser"><div className="column-browser-heading"><div><p className="panel-kicker">{selectedSchema && selectedTable ? `${selectedSchema}.${selectedTable}` : "Select a table"}</p><h2>Columns</h2></div><Badge tone="neutral">{columns.length} columns</Badge></div><div className="columns-table"><div className="columns-head"><span>Column</span><span>Type</span><span>Nullable</span></div>{busyAction === "columns" ? <LoadingRows count={3} /> : visibleColumns.map((column) => <div className="columns-row" key={column.name}><span className="column-name">{column.primaryKey ? <span className="key-marker" title="Primary key">PK</span> : null}{column.name}</span><span className="column-type">{column.type ?? column.dataType ?? "Unknown"}</span><span>{column.nullable ? "Yes" : "No"}</span></div>)}</div><Pagination {...columnPagination} /><div className="column-footer"><span>Selected source table</span><Button variant="secondary" size="sm" onClick={onImport} icon={DownloadSimple} disabled={!selectedTable}>Import table</Button></div></section></div></div>;
}
...
function FileIndexPage({ title, description, files, pattern, icon: Icon, emptyTitle, emptyBody, onOpenFile }: {
  ...
  const { pageItems, paginationProps } = usePagination(matches, pattern.source);
  return <div className="page"><SectionHeading eyebrow="Semantic layer" title={title} description={description} /><section className="panel table-panel"><div className="list-toolbar"><div><strong>{matches.length} files</strong><span>indexed from the project API</span></div><Badge tone="neutral">Read only</Badge></div>{matches.length === 0 ? <EmptyState icon={Icon} title={emptyTitle} body={emptyBody} /> : <><div className="data-table"><div className="data-table-head"><span>Name</span><span>Path</span><span>Size</span><span>Status</span><span aria-hidden="true" /></div>{pageItems.map((file) => <div className="data-table-row" key={file.path}><span className="name-cell"><span className="row-icon"><Icon size={16} /></span><span><strong>{fileName(file.path)}</strong><small>Source file</small></span></span><span className="muted-code">{file.path}</span><span>{file.size ? `${Math.max(1, Math.round(file.size / 1024))} KB` : "Not available"}</span><span><Badge tone={file.draft ? "amber" : "neutral"} dot>{file.draft ? "Draft" : "Tracked"}</Badge></span><Button variant="ghost" size="sm" onClick={() => onOpenFile(file.path)}>Open source <ArrowRight size={14} /></Button></div>)}</div><Pagination {...paginationProps} /></>}</section><div className="info-strip"><Info size={17} /><span>Structure is indexed from real project files. Use MDL source to edit and save a file through the project API.</span></div></div>;
}
...
function VersionHistory({ versions, onRollback, busyAction }: { versions: VersionRecord[]; onRollback: (version: VersionRecord) => void; busyAction: string | null }) {
  const { pageItems, paginationProps } = usePagination(versions);
  return <div className="version-history"><div className="version-list">{pageItems.map((version) => { const current = versions[0]?.id === version.id; return <div className="version-row" key={version.id}><span className={`version-marker ${current ? "version-marker-current" : ""}`}>{current ? <Check size={13} /> : <GitBranch size={13} />}</span><span className="version-copy"><strong>{version.label ?? version.revision}</strong><small>{version.revision} <span>/</span> {formatDate(version.createdAt)} <span>/</span> {version.fileCount ?? 0} files</small></span>{current ? <Badge tone="blue">Current</Badge> : <Button variant="ghost" size="sm" onClick={() => onRollback(version)} loading={busyAction === `rollback-${version.id}`}>Restore</Button>}</div>; })}</div><Pagination {...paginationProps} /></div>;
}
