...
const copy = {
  "en-US": {
    ...
    accounts: "Service accounts", policies: "Policies", audit: "Audit", refresh: "Refresh", create: "Create account", accountName: "Account name", regions: "Region codes", regionsHint: "Comma-separated trusted attributes, for example CN-JIA,CN-YI.",
    ...
    bind: "Bind policy", choosePolicy: "Choose a policy", bound: "Bound policies", shownOnce: "Copy this key now", shownOnceBody: "SemaRail stores only its hash. Closing this notice permanently hides the plaintext.", copied: "Copied", closeKey: "I saved the key",
  ...
  "zh-CN": {
    ...
    accounts: "服务账号", policies: "权限策略", audit: "审计", refresh: "刷新", create: "创建账号", accountName: "账号名称", regions: "地区代码", regionsHint: "受信任属性，使用英文逗号分隔，例如 CN-JIA,CN-YI。",
    ...
    bind: "绑定策略", choosePolicy: "选择策略", bound: "已绑定策略", shownOnce: "请立即复制该密钥", shownOnceBody: "SemaRail 只保存哈希；关闭提示后将无法再次查看明文。", copied: "已复制", closeKey: "我已保存密钥",
...
export default function AccessControl({ locale }: { locale: Locale }) {
  const c = copy[locale];
  ...
  async function loadAll(adminToken = token) {
    ...
    try {
      const [accountResult, policyResult, auditResult] = await Promise.all([
        api.getServiceAccounts(adminToken), api.getAccessPolicies(adminToken), api.getAccessAudit(adminToken),
      ]);
      setAccounts(accountResult.items); setPolicies(policyResult.items); setAudit(auditResult.items);
      setSelectedId((current) => accountResult.items.some((item) => item.id === current) ? current : accountResult.items[0]?.id ?? "");
      setToken(adminToken); setTokenInput("");
    } catch (caught) {
  ...
  return <div className="page access-page">
    ...
    <div className="access-tabs" role="tablist">{(["accounts", "policies", "audit"] as Tab[]).map((item) => <button key={item} role="tab" aria-selected={tab === item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{c[item]}</button>)}</div>
