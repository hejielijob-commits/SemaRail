...
import { Badge, Button, EmptyState, Field, InlineNotice, LoadingRows, Pagination, SectionHeading, Select, TextArea, TextInput, usePagination } from "./ui";
...
export default function AccessControl({ locale, onAuthenticated, adminToken = "", activeDatasourceId = "" }: { locale: Locale; onAuthenticated?: () => void; adminToken?: string; activeDatasourceId?: string }) {
  ...
  const { pageItems: visibleAccounts, paginationProps: accountPagination } = usePagination(accounts);
  const { pageItems: visibleUsers, paginationProps: userPagination } = usePagination(users);
  const { pageItems: visiblePolicies, paginationProps: policyPagination } = usePagination(policies);
  const { pageItems: visibleAudit, paginationProps: auditPagination } = usePagination(audit);
  const { pageItems: visibleCredentials, paginationProps: credentialPagination } = usePagination(selected?.credentials ?? [], selected?.id ?? "");
