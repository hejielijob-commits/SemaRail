...
const COPY = {
  "en-US": {
    ...
    title: "Views",
    description: "Build reusable business datasets with Wren SQL, then inspect their source and runtime output before publishing.",
    ...
    empty: "No views yet",
    emptyBody: "Create a reusable Wren view to shape a business dataset without changing the source models.",
    ...
    statement: "View SQL",
    statementHint: "One read-only query. Reference Wren models directly; nested View references are not supported by the current Wren 0.13.2 Python runtime.",
    ...
    createStatementHint: "Use a Wren model name, for example SELECT * FROM orders.",
    cancel: "Cancel",
    ...
    previewBody: "Wren Views do not declare output columns. Run a bounded, read-only PostgreSQL preview to discover the actual fields and sample rows.",
    ...
    previewUnavailableBody: "Connect a supported database and enable the safe Wren query runtime. No sample rows have been fabricated.",
    noPreview: "Preview has not run",
    noPreviewBody: "Run the View through Wren to inspect its database-derived columns and rows.",
    rows: (count: number) => `${count} rows`,
    truncated: "Bounded result",
    nullValue: "NULL",
    dependencyTitle: "SQL references",
    dependencyBody: "References are parsed from FROM and JOIN clauses. Wren planning remains the authoritative semantic check.",
    ...
    view: "View",
    unknown: "Unresolved",
    nestedUnsupported: "Nested View reference is not supported in the current Wren Python runtime.",
  ...
  "zh-CN": {
    eyebrow: "语义层",
    title: "视图",
    description: "使用 Wren SQL 构建可复用的业务数据集，并在发布前检查源码和真实运行结果。",
    ...
    empty: "暂无视图",
    emptyBody: "创建可复用的 Wren 视图，在不改动源模型的情况下组织业务数据集。",
    select: "选择一个视图",
    ...
    statement: "视图 SQL",
    statementHint: "只允许一条只读查询。请直接引用 Wren 模型；当前 Wren 0.13.2 Python 运行时不支持嵌套引用 View。",
    dirty: "有未保存修改",
    ...
    createStatementHint: "请使用 Wren 模型名称，例如 SELECT * FROM orders。",
    cancel: "取消",
    ...
    previewBody: "Wren View 本身不声明输出字段。运行有边界的只读 PostgreSQL 预览，才能获得真实字段与样例数据。",
    runPreview: "运行预览",
    runningPreview: "正在预览",
    retryPreview: "重试预览",
    previewFailed: "预览失败",
    previewUnavailable: "运行时预览不可用",
    previewUnavailableBody: "请连接受支持的数据库并启用安全 Wren 查询运行时。这里不会伪造任何样例数据。",
    noPreview: "尚未运行预览",
    noPreviewBody: "通过 Wren 运行该 View，检查数据库推断出的字段和数据行。",
    rows: (count: number) => `${count} 行`,
    truncated: "结果已限制",
    nullValue: "空值",
    dependencyTitle: "SQL 引用",
    dependencyBody: "引用来自 FROM 和 JOIN 子句的静态解析，Wren 规划结果仍是最终语义校验依据。",
    noDependencies: "未发现引用",
    noDependenciesBody: "在 FROM 或 JOIN 子句中添加模型引用，然后校验视图。",
    model: "模型",
    view: "视图",
    unknown: "未解析",
    nestedUnsupported: "当前 Wren Python 运行时不支持嵌套 View 引用。",
    metadataFile: "metadata.yml",
    sqlFile: "sql.yml",
